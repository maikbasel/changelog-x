...
fn run_cgx_with_env(env_vars: &[(&str, &str)]) -> String {
...
fn test_env_ai_provider() {
    let output = run_cgx_with_env(&[("CGX_AI__PROVIDER", "openai")]);
    assert!(
...
fn test_env_changelog_output() {
    let output = run_cgx_with_env(&[("CGX_CHANGELOG__OUTPUT", "RELEASES.md")]);
    assert!(
